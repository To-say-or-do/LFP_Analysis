...
for k = 1:k_t 
    ...
    for l = 1:length(contrastValues)
        plot(mean(Con_Resps{l}(S_Area,:)*3e04,1),'Color',Color_L4{l},'DisplayName',Label_L4{l},'LineWidth',1);
    end
    hold off
    xlabel('time(s)');
    ylabel('electrode');
...
%     set(gca,'YDir','reverse');
    ...
    pbaspect([1 1 1]);
    title_name=sprintf('L4-Plot %s %d %d',files(i).name(1:6),mean(L4_electrodes{i})-10*(k-(k_t+1)/2)-5,mean(L4_electrodes{i})-10*(k-(k_t+1)/2)+4);
    title(title_name);
